...
static void assert_matrix_close(const RMatrix *matrix, const float *expected, size_t count, float abs_tol,
                                float rel_tol, const char *label)
{
    for (size_t i = 0; i < count; i++)
    {
        ...
        assert_float_close(matrix->data[i], expected[i], abs_tol, rel_tol, msg);
    }
}
...
static void test_relu_basic_values(void)
{
    ...
    assert_true(output->rows == 2 && output->cols == 3, "relu_basic preserves shape");
    assert_matrix_close(output, expected_vals, 6, DEFAULT_ABS_TOL, DEFAULT_REL_TOL, "relu_basic");
...
static void test_relu_preserves_input_and_allocates_new(void)
{
    ...
    assert_true(output->data != input->data, "relu does not alias input data");

    for (size_t i = 0; i < 4; i++)
    {
        char msg[96];
        snprintf(msg, sizeof(msg), "relu_input_unchanged idx=%zu", i);
        assert_float_close(input->data[i], input_vals[i], DEFAULT_ABS_TOL, DEFAULT_REL_TOL, msg);
    }
...
static void test_relu_zero_sizes(void)
{
    ...
    assert_true(output_rows0->rows == 0 && output_rows0->cols == 3, "relu_zero_rows preserves shape");
    ...
    assert_true(output_cols0->rows == 3 && output_cols0->cols == 0, "relu_zero_cols preserves shape");
...
static void test_relu_nan_converts_to_zero(void)
{
    ...
    assert_matrix_close(output, expected_vals, 3, DEFAULT_ABS_TOL, DEFAULT_REL_TOL, "relu_nan");
...
static void test_gelu_known_values(void)
{
    ...
    const float expected_vals[] = {0.0f, 0.8413447f, -0.1586553f, 1.9544997f, -0.045500264f};

    RMatrix *input = make_matrix(1, 5, input_vals);
    RMatrix *output = r_activation_gelu(input);

    assert_matrix_close(output, expected_vals, 5, DEFAULT_ABS_TOL, DEFAULT_REL_TOL, "gelu_known");
...
static void test_gelu_preserves_input_and_shape(void)
{
    ...
    assert_true(output->rows == 2 && output->cols == 2, "gelu preserves shape");
    assert_true(output != input, "gelu returns new matrix");
    assert_true(output->data != input->data, "gelu does not alias input data");
...
static void test_gelu_zero_sizes(void)
{
    ...
    assert_true(output_rows0->rows == 0 && output_rows0->cols == 4, "gelu_zero_rows preserves shape");
    r_free_matrix(input_rows0);
    r_free_matrix(output_rows0);

    RMatrix *input_cols0 = r_create_matrix(4, 0);
    RMatrix *output_cols0 = r_activation_gelu(input_cols0);
    assert_true(output_cols0->rows == 4 && output_cols0->cols == 0, "gelu_zero_cols preserves shape");
    r_free_matrix(input_cols0);
    r_free_matrix(output_cols0);
}
...
static void test_gelu_large_magnitude_limits(void)
{
    ...
    assert_float_close(output->data[0], 6.0f, 1e-4f, DEFAULT_REL_TOL, "gelu_large_positive");
    assert_float_close(output->data[1], 0.0f, 1e-4f, DEFAULT_REL_TOL, "gelu_large_negative");
...
static void test_softmax_basic_values(void)
{
    ...
    assert_matrix_close(output, expected_vals, 3, DEFAULT_ABS_TOL, DEFAULT_REL_TOL, "softmax_basic");
...
static void test_softmax_input_unchanged(void)
{
    ...
    {
        char msg[96];
        snprintf(msg, sizeof(msg), "softmax_input_unchanged idx=%zu", i);
        assert_float_close(input->data[i], input_vals[i], DEFAULT_ABS_TOL, DEFAULT_REL_TOL, msg);
    }
