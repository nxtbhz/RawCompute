...
static RLayerDense *make_layer(size_t n_inputs, size_t n_neurons, const float *weights, const float *biases)
{
    ...
    {
        memcpy(layer->weights->data, weights, sizeof(float) * weight_count);
    }
...
static void assert_matrix_close(const RMatrix *actual, const float *expected, size_t rows, size_t cols, const char *msg)
{
    ...
    for (size_t i = 0; i < rows; i++)
    {
        ...
        {
            const size_t idx = RMatrixIDX(i, j, actual->cols);
            char buf[128];
            snprintf(buf, sizeof(buf), "%s idx=%zu", msg, idx);
            assert_float_close(actual->data[idx], expected[idx], DEFAULT_ABS_TOL, DEFAULT_REL_TOL, buf);
        }
    }
}
...
static void test_create_layer_shapes(void)
{
    ...
    assert(layer->weights != NULL && layer->biases != NULL && "layer members allocation failed");
    assert(layer->weights->rows == 3 && layer->weights->cols == 4 && "weights shape mismatch");
...
static void test_layer_forward_single_neuron(void)
{
    ...
    assert(output->rows == 1 && output->cols == 1 && "single neuron output shape mismatch");
    assert_float_close(output->data[0], 2.3f, DEFAULT_ABS_TOL, DEFAULT_REL_TOL, "single neuron output");
...
static void test_layer_forward_single_input_three_neurons(void)
{
    ...
    assert_matrix_close(output, expected, 1, 3, "single input three neurons");
...
static void test_layer_forward_zero_rows_no_crash(void)
{
    ...
    assert(output->rows == 0 && output->cols == 2 && "zero-row output shape mismatch");
...
static void test_layer_forward_nan_propagation(void)
{
    ...
    assert(isnan(output->data[0]) && "nan inputs should propagate to output");
