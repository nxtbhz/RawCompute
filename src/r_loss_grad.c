...
void r_cross_entropy_grad(const RMatrix *pred, const RMatrix *real, RMatrix *grad)
{
...
void r_bin_cross_entropy_grad(const RMatrix *pred, const RMatrix *real, RMatrix *grad)
{
...
void r_cat_cross_entropy_grad(const RMatrix *pred, const RMatrix *real, RMatrix *grad)
{
    float norm = (float)pred->rows;
...
void r_mae_loss_grad(const RMatrix *pred, const RMatrix *real, RMatrix *grad)
{
    ...
    for (size_t i = 0; i < pred->rows; i++)
    {
        for (size_t j = 0; j < pred->cols; j++)
        {
            float diff = pred->data[RMatrixIDX(i, j, pred->cols)]
                       - real->data[RMatrixIDX(i, j, real->cols)];
                           ...
                           float gamma, float alpha, RMatrix *grad)
{
    ...
    for (size_t i = 0; i < pred->rows; i++)
    {
        for (size_t j = 0; j < pred->cols; j++)
        {
            ...
            float d_t1 = alpha * y * (
                -gamma * powf(omp, gamma - 1.0f) * lp
                + powf(omp, gamma) / (p + EPSILON)
            );

            float d_t2 = (1.0f - alpha) * (1.0f - y) * (
                gamma * powf(p, gamma - 1.0f) * lop
                - powf(p, gamma) / (omp + EPSILON)
            );
    ...
    for (size_t i = 0; i < pred->rows; i++)
    {
        for (size_t j = 0; j < pred->cols; j++)
        {
            ...
            if (y > 0.0f)
            {
                ...
                float omp = 1.0f - p;
                g = -(y / norm) * (
                    -gamma * powf(omp, gamma - 1.0f) * logf(p + EPSILON)
                    + powf(omp, gamma) / (p + EPSILON)
                );
            }
